...
# Local
import benchmark.DiffJPEG.utils as utils
...
class rgb_to_ycbcr_jpeg(nn.Module):
    ...
    def forward(self, image):
        ...
        result = torch.tensordot(image.float(), self.matrix, dims=1) + self.shift
...
class dct_8x8(nn.Module):
    ...
    def __init__(self):
        ...
        #
        self.tensor = nn.parameter.Parameter(torch.from_numpy(tensor).float())
...
class y_quantize(nn.Module):
    ...
    def __init__(self, rounding, quantization_table):
        super(y_quantize, self).__init__()
        self.rounding = rounding
        self.y_table = torch.tensor(quantization_table[0]).reshape(8, 8).t()

    def forward(self, image):
        image = image.float() / self.y_table.to(image.device)
...
class c_quantize(nn.Module):
    ...
    def __init__(self, rounding, quantization_table):
        ...
        self.rounding = rounding
        self.c_table = torch.tensor(quantization_table[1]).reshape(8, 8).t()

    def forward(self, image):
        image = image.float() / self.c_table.to(image.device)
        return self.rounding(image)
...
class compress_jpeg(nn.Module):
    ...
    def __init__(self, quantization_table, rounding_c, rounding_y):
        ...
        self.c_quantize = c_quantize(rounding_c, quantization_table)
        self.y_quantize = y_quantize(rounding_y, quantization_table)
