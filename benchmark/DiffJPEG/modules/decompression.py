...
class y_dequantize(nn.Module):
    ...
    def forward(self, image):
        return image * self.y_table.to(image.device)
...
class c_dequantize(nn.Module):
    ...
    def forward(self, image):
        return image * self.c_table.to(image.device)
...
class block_merging(nn.Module):
    ...
    def forward(self, patches, height, width):
        ...
        return image_transposed.contiguous().reshape(batch_size, height, width)
...
class decompress_jpeg(nn.Module):
    ...
    def __init__(self, quantization_table):
        ...
        self.c_dequantize = c_dequantize(quantization_table)
        self.y_dequantize = y_dequantize(quantization_table)
        self.idct = idct_8x8()
    ...
    def forward(self, y, cb, cr):
        ...
        full_height, full_width = y.shape[-2] * math.isqrt(y.shape[1]), y.shape[-1] * math.isqrt(y.shape[1])
        ...
        if "KEEP_DIFF_JPEG_CLAMP" in os.environ:
            image = torch.min(
